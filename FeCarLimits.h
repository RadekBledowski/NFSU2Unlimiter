...
// ---------------------------------------------------------------------------------------------
// Keeping ECX across a replaced accessor
//
// The game's own FEPlayerCarDB accessors never write to ECX. They use the this pointer right
// through the body and return with it untouched, and callers exploit that.
// check_quick_race_unlock at 0x529CC0 is the one that bit us:
//
//     mov  ecx, <PlayerCareer>
//     push eax
//     call FEPlayerCarDB::GetCarRecordByHandle   ; 0x503510, never touches ecx
//     mov  esi, [ecx+540h]                       ; and reads it again here
//
// A compiled C++ replacement uses ECX as scratch like any other register, so that read lands on
// whatever its loop left behind. In three crash logs it was 0, giving an access violation
// reading 0x540. It only showed up with ExtendFeCarLimits on, and turning UnlockAllThings back
// on hid it, because UnlockSystem::IsCarUnlocked returns at its first instruction when
// everything is unlocked and never reaches that call.
//
// So every replacement of one of these accessors goes through a shim that puts ECX back. This
// cannot change behaviour: either a caller depends on ECX surviving, in which case the vanilla
// function preserved it and so must we, or it does not, in which case restoring it is invisible.
//
// Proven for GetCarRecordByHandle. The others are the same shape with the same callers, and the
...
// believing any of these: a shim that cleans the wrong amount corrupts the stack silently.
